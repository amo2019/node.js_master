...
handlers._users.post = function (data, callback) {
  ...
  var tosAgreement =
    ...
    data.payload.tosAgreement == true
      ...
      : false;
  console.log(
    "firstName && lastName && email && password && tosAgreement:",
    firstName,
    lastName,
    address,
    email,
...
handlers._users.get = function (data, callback) {
  ...
  var email =
    ...
    data.queryStringObject.email.trim().length > 7
      ...
      : false;
  console.log(
    "data.queryStringObject.email.trim(), data.headers.token",
    data.queryStringObject.email.trim(),
    data.headers.token
  );
  if (email) {
      ...
      if (tokenIsValid) {
        ...
        _data.read("users", email, function (err, data) {
          if (!err && data) {
            ...
            console.log("_users.getdata:", data);
        ...
        console.log("x2header, data", data);
        callback(403, {
    ...
    if (firstName || lastName || address || password) {
      ...
      handlers._tokens.verifyToken(token, email, function (tokenIsValid) {
        if (tokenIsValid) {
          ...
          console.log("x3header, data", data);
          callback(403, {
            Error: "Missing required token in header, or token is invalid.",
          });
        }
...
handlers._stores.get = function (data, callback) {
  console.log("hitting get /store:", data);
  var baseDir = path.join(__dirname, "/../.data/");
  console.log("Dir:", baseDir + "data" + "/" + "items" + ".json");
  fs.readFile(
    baseDir + "data" + "/" + "items" + ".json",
    "utf8",
    function (error, fileData) {
      if (error) {
        console.log("Dir:", `lib.baseDir + data + "/" + items + ".json"`);
        console.log("couldn not read a file 864");
        callback(500, { Error: "couldn not read items file." });
      } else {
        console.log("data.headers", data.headers.useremail);
        var userEmail = data.headers.useremail;
        console.log("purchases userEmail", userEmail);
        ...
        _data.read("purchases", userEmail, function (err, purchaseData) {
          if (err) {
            ...
            var itemsToBuy2 = [];
            //console.log("itemsToBuy``", itemsToBuy[1].id, itemsPurchase[1].id);
              ...
              itemsPurchase.forEach((item) => {
                ...
                for (let item2 of itemsToBuy) {
                  if (item.id === item2.id) {
                    ...
                    console.log("item.id, item2.id:", item.id, item2.id);
                    break;
                  }
                }
              });
            ...
            console.log("purchases", fileData, purchaseData);
            callback(200, {
              // stripePublicKey: stripePublicKey,
...
handlers._purchases.add = function (data, callback) {
  ...
  console.log("data.payload.userEmail::", data.userEmail);
  var userEmail =
  ...
  if (data) {
    ...
    _data.read("purchases", userEmail, function (err, userData) {
      console.log("err, userDatan::", err, userData);
      if (!err) {
    ...
    console.log("purchase5, data", data.items);
  }
};
// ------------------------------------

handlers._purchases.post = function (data, callMeback) {
  console.log("Hiting Purchase Route (post)..");
  console.log("data:-", data);
  let sendMail = function (email = "aminvomar@gmail.com", subject, text, cb) {
    ...
    const request = https.request(options, (res) => {
      ...
      res.on("data", function (body) {
        dataToCollect = +body;
        console.log("dataxx:", dataToCollect);
      });
      cb(res);
    });
  ...
  if (id && email) {
    ...
    _data.read("tokens", email, function (err, tokenData) {
      if (!err && tokenData) {
        ...
        callback(404);
        console.log("tokenData Error::", tokenData, data);
      }
    });
  } else {
    callback(400, { Error: "Missing required field, or field invalid" });
    console.log("x6, data email", data);
  }
};
...
handlers._tokens.put = function (data, callback) {
  ...
  var extend =
    typeof data.payload.extend == "boolean" && data.payload.extend == true
      ? true
      : false;
  console.log("x7 id, extend, email", id, extend, data.payload.email);

  var email = data.payload.email.trim();
  console.log("data.payload.email:", data);
  // Check that email number is valid
  ...
  _data.read("tokens", email, function (err, tokenData) {
    if (!err && tokenData) {
      ...
      if (tokenData.id == id && tokenData.expires > Date.now()) {
        console.log("tokenData.id, id:", tokenData.id, id);
        callback(true);
      } else {
        callback(false);
      }
