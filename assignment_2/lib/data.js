    ...
    for (var i in files) {
      ...
      if (
        ...
        if (idx >= 0) {
          ...
          console.log("currentFileBody:", currentFileBody);
          console.log(
            "currentFileBody.email == email:",
            currentFileBody.email,
            email
          );
          ...
          if (!tokenId && currentFileBody.email == email) {
            ...
            if (currentFileBody._id) {
              fileData.email.password = true;
              console.log(
                "currentFileBody._id, hashedPassword::::",
                currentFileBody._id,
                hashedPassword
              );
            }
            // var value = currentFileBody
            //   .match(/([a-zA-Z0-9._+-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi)
            //   .toString()
            //   .replace(/[']/g, "");
            // console.log("value", value);
            // fileData.email.email = value;
          ...
          // var emailValue = currentFileBody
          //   .match(/([a-zA-Z0-9._+-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi)
          //   .toString()
          //   .replace(/[']/g, "");
          // console.log(
          ...
          // fileData.token.email = true;
          try {
            //fileData.token.expired = currentFileBody.match(/exp=(\d+)/i)[1];
            // Regex is slower but safer
            //let parsedFileBody = JSON.parse(currentFileBody);
            ...
            if (currentFileBody.exp < Math.floor(Date.now() / 1000)) {
              console.log(
                "currentFileBody.exp, currenDate:",
                currentFileBody.exp,
                currenDate
              );
...
lib.create = function (dir, file, data, callback) {
  ...
  if (data.method !== "get") {
    ...
    fs.open(
      ...
      "wx",
      function (err, fileDescriptor) {
        // console.log("err, fileDescriptor:", err, fileDescriptor);
        if (!err && fileDescriptor) {
          // Convert data to string
          var stringData = JSON.stringify(data);
  ...
  fs.readFile(
    ...
    function (err, data) {
      if (!err && data) {
        ...
        console.log("hashedfile2", hashedfile);
        callback(err, data);
      }
    }
  );
